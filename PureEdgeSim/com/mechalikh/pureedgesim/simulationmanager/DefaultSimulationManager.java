...
import com.mechalikh.pureedgesim.taskgenerator.ParentTask;
...
public class DefaultSimulationManager extends SimulationManager {
	...
	protected List<ParentTask> parentTasks = new ArrayList<ParentTask>();
	...
	public void startInternal() {
		...
		for (int i = 0; i < Math.min(taskList.size(), SimulationParameters.batchSize); i++) {
			for (int j = 0; j < taskList.first().getSubTasks().size(); j++) {
				schedule(this, taskList.first().getSubTasks().get(j).getTime() - simulation.clock(), SEND_TO_ORCH,
						taskList.first().getSubTasks().get(j));
			}
			parentTasks.add(taskList.first());
	...
	public void processEvent(Event ev) {
		...
		case TRANSFER_RESULTS_TO_ORCH:
			...
			// Send the results of this subtask to the next one (as input), if any
			if (((SubTask) task).getDependentTasks().size() > 0) {
				for (int i = 0; i < ((SubTask) task).getDependentTasks().size(); i++) {
					task.setOrchestrator(((SubTask) task).getDependentTasks().get(i).getOffloadingDestination());
		...
		case RESULT_RETURN_FINISHED:
			...
			if (!((SubTask) task).hasBeenReturned()) {
				((SubTask) task).hasBeenReturned(true); 
				System.out.println("######"+this.getClass().getSimpleName()+", SubTask: "+((SubTask) task).getParentTask().getId()+"-"+ ((SubTask) task).getId() +"  has been executed");
				// The task A that is required for task B has been finished, we can now execute
				// task B
				for (int i = 0; i < ((SubTask) task).getDependentTasks().size(); i++) {
					
					if (((SubTask) task).getDependentTasks().get(i).isWaiting())
						((SubTask) task).getDependentTasks().get(i).getOffloadingDestination()
								.submitTask(((SubTask) task).getDependentTasks().get(i));
	...
	protected boolean setFailed(Task task, int phase) {
		// if one subtask fails, the parent task fail
		((SubTask) task).getParentTask().setFailureReason(task.getFailureReason());
		System.err.println(this.getClass().getSimpleName()+", ParentTask: "+ ((SubTask) task).getParentTask().getId()+" is now considered failed, because one of its subtasks ("+((SubTask) task).getParentTask().getId()+"-"+((SubTask) task).getId()+"-"+")has failed");
	...
	public void onSimulationEnd() {
		...
		for (int i = 0; i < this.parentTasks.size(); i++) {
			total++;
			if (this.parentTasks.get(i).getStatus() == Status.SUCCESS)
				success++;
		}
		System.err.println("Success rate is: " + (success / total * 100));
	}
